...
class GoodsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        start_urls = ['https://search.jd.com/Search?keyword=手机&enc=utf-8&page={}'.format(str(i)) for i in range(1,2,2)]
...
#.//*[@id='J_goodsList']/ul/li/div/div[4]/a/em
#//li[@class="gl-item"]/div/div[@class="p-name p-name-type-2"]/a/@title
    def parse(self, response):
        item = DataserchItem()
        item['ititle'] = response.xpath('//*[@id=''J_goodsList'']/ul/li/div/div[4]/a/em/text()').extract()
        item['iprice'] = response.xpath("//div[@class='p-price']/strong/i/text()").extract()
        item['iurl'] = response.xpath('//div[@class="p-name p-name-type-2"]/a[@target="_blank"]/@href').extract()
        item['icomment_num'] = response.css('.p-commit a::text').extract()
        print(item)
        yield item
